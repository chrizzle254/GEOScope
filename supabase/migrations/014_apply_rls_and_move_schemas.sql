...
create policy "Members can view mentions" on public.mentions for select using (analysis_run_id in (select id from public.analysis_runs where reporting_subject_id in (select id from public.reporting_subject where organization_id in (select organization_id from public.organization_members where user_id = auth.uid()))));
create policy "Owners/admins can manage mentions" on public.mentions for all using (analysis_run_id in (select id from public.analysis_runs where reporting_subject_id in (select id from public.reporting_subject where organization_id in (select organization_id from public.organization_members where user_id = auth.uid() and role in ('owner', 'admin')))));
